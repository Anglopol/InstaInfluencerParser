...
namespace InfluencerInstaParser.Manager
{
    public class InfluenceManager
    {
        ...
        public List<Location> GetListOfLocationsFromUserById(string userId)
        {
            ...
            return Neo4JClientHandler.GetListOfLocationsFromTarget(_graphClient, dateOfParsing, user.Username);
        }
        ...
        private void FillDatabase(string targetUsername, IEnumerable<User> users, IEnumerable<Location> locations)
        {
            ...
            Neo4JClientHandler.CreateTarget(_graphClient, targetUsername, out var targetId);
            Neo4JClientHandler.CreateAnalysis(_graphClient, _currentDate, targetId, out var analysisId);
